...
function enableButtons () {
	...
	// Set the onclick action for the second button
	btn = document.getElementById("input_greet_by_date");
	btn.onclick=function(){ greetByPeriod();};
	
	// Update the button label now that the button is active
	btn.value="Click me for a cool greeting";
...
function getCount(){
	var request = gapi.client.helloworldendpoints.getCount();
	request.execute(sayCount);
}

function sayCount(response)
{
	alert(response.saycount);
}
